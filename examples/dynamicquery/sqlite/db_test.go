...
func TestListRecordsSQLite(t *testing.T) {
...
func TestSearchContactsSQLite(t *testing.T) {
	sdb, cleanup := sqltest.SQLite(t, []string{"schema.sql"})
	defer sdb.Close()
	defer cleanup()
...
func TestExcludeContactsSQLite(t *testing.T) {
	sdb, cleanup := sqltest.SQLite(t, []string{"schema.sql"})
	defer sdb.Close()
	defer cleanup()
...
func TestFilterRecordsSQLite(t *testing.T) {
	sdb, cleanup := sqltest.SQLite(t, []string{"schema.sql"})
	defer sdb.Close()
	defer cleanup()
